...
path1="/mnt/syno8/data/Hollywood2/AVIClips"  # path of the raw file
path2="/mnt/scratch/mikhail/salieny_cnn_data/__3DCNN_preprocessed__/Hollywood_trainh5_1"  # path of the video after process
...
# interation over each video
for videos in sorted(glob.glob('actioncliptrain*.avi')):
    ...
    del all_framesx
    del all_framesy
    hw_train = util.datasets.load_hollywood2_train(verbose=True, out_size=util.Size(72*2, 128*2))
    
    file_name= hw_train['ground_truth'][video_num]['filename']
    file_name=file_name.split('/')[-1]   # the last '/' in the file name is the video name
    file_name=file_name.split('_')[0]
    assert videos.split('.')[0]==file_name
    
    augmented_points, augmented_labels = util.GroundTruthHandler.augment_with_negative_samples(hw_train['ground_truth'][video_num]) 
...
# End of interation over train video
'''

video_num=0

for videos in sorted(glob.glob('actioncliptest*.avi')):
    video=cv2.VideoCapture(path1+'/'+videos)  # read one video
    if video is None:
       print ("This is the last video")
       break
    ...
    file_name= gc['video_parameters'][video_num]['filename']
    file_name=file_name.split('/')[-1]   # the last '/' in the file name is the video name
    file_name=file_name.split('.')[0]
    print videos.split('.')[0]
    print str(file_name)
    assert videos.split('.')[0]==str(file_name)
